...
public class LambdaGenerateToken
  ...
  public APIGatewayProxyResponseEvent handleRequest(
    ...
    logger.log("GenerateToken: Processing request");
    ...
    try {
      ...
      if (!email.matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
        responseJson.put("error", "Invalid email format");
        return createErrorResponse(headers, responseJson, 400);
      }

      logger.log("Generating token for email: " + email);
      ...
      if (token == null) {
        responseJson.put("error", "Failed to generate token");
        return createErrorResponse(headers, responseJson, 500);
      }
      ...
      responseJson.put("generated_at", Instant.now().toString());
      responseJson.put("message", "Token generated successfully");
      ...
      logger.log("Exception in GenerateToken: " + e.getMessage());
      responseJson.put("error", "Failed to generate token: " + e.getMessage());
      return createErrorResponse(headers, responseJson, 500);
    }
  }
  ...
  private static String generateSecureToken(String data, String key, LambdaLogger logger) {
    try {
      ...
      logger.log("Generated token for: " + data);
      return token;
